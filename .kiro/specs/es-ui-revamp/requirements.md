...
8. THE Estimates List page SHALL provide a Delete action on each estimate row (table) and card (mobile) that, WHEN activated, opens a confirmation dialog (`ConfirmDialog`) naming the estimate; the estimate is removed only WHEN the user confirms. On confirm, THE page SHALL call `DELETE /api/v1/estimates/:id` (server soft-delete via `deletedAt`), remove the row from the visible list, and on failure surface an error without removing the row. Cancelling or dismissing the dialog SHALL leave the estimate unchanged.
...
7. THE Estimate Editor page SHALL present two explicit save controls — **Save draft** (preserves in-progress work, sets `status='draft'`, requires only that at least one layer with a selected material exist) and **Save** (commits the estimate, sets `status='sent'` in the DB enum, requires full configurator validation to pass via `validateConfiguredEstimate`).
8. WHEN a user activates the Back or Close control on the Estimate Editor page, THE Estimate Editor page SHALL silently persist the current state to the server (preserving the existing `status` if the row is already saved; defaulting to `status='draft'` for new rows) and SHALL NOT present any confirmation dialog before navigating away.
9. IF the silent save on Back fails because the user has not yet added a layer or selected a material on a brand-new estimate, THEN THE Estimate Editor page SHALL place a one-shot flash notice (`"Nothing saved yet — add at least one layer to save a draft."`) into session storage under the key `es:flashNotice` and navigate to the return route; the Estimates List page SHALL read this notice once on mount, render it as a dismissible banner, and remove the storage entry so the same notice never appears twice.
10. THE Estimate Editor page SHALL expose a free-text **Notes** textarea (up to 2000 characters) inside the Job details card, bound to the `estimates.notes` column, included in every save payload (empty string when cleared), and rendered on subsequent loads of the same estimate.
11. THE Estimates List page SHALL render the estimate status badge using the label vocabulary "Draft" (for `status='draft'`) and "Saved" (for `status='sent' | 'won' | 'lost'`), and SHALL provide separate "Drafts" and "Saved" filter chips so the two states are distinguishable.
12. WHERE the `VITE_ENABLE_MES_OUTCOME` environment flag is not set to `'true'`, THE Estimate Editor page SHALL hide the Mark Won / Mark Lost controls and THE Dashboard page SHALL hide the Won KPI tile; the underlying DB enum and code paths remain intact so the MES integration can enable both surfaces by flipping the flag without further code changes.
