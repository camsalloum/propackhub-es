...
3. **Product-type / subtype model is decoupled and Master-Data-driven — refines §18.1.** Rules for agents:
   - **Superseded 2026-07-04:** Bag and Pouch are first-class in DB and engine (`roll | sleeve | pouch | bag`). `engineTypeForFamily()` is identity for those four codes (no bag→pouch collapse).
